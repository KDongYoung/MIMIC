...
class BasicBlock(nn.Module):
    ...
    def __init__(self, d, d_hidden, norm_layer, dr, activation, track_running=True):
        ...
        self.bn0 = norm_layer(d_hidden, track_running_stats=track_running) 
        self.bn1 = norm_layer(d, track_running_stats=track_running)     
    ...
    def forward(self, x):
        ...
        out = self.activation(self.bn0(self.fc1(x)))
        # out = self.dropdout1(self.activation(out))
        out = self.dropdout0(self.bn1(self.fc2(out)))
        out += identity
...
def resnet_18(args):
    ...
    return Resnet_org(args, n_layers, d, d_hidden_factor, dr, activation)
