...
def start_Training(args):
    ...
    for i, (train_index, test_index) in enumerate(kfold_split): 
        ...
        trainer = Trainer(args, model, i+1, model_name=args['model_name'])
        ...
        if args['model_type'] =="DL":
            for metric in args['eval_metric']:
                ...
                best_trainer = Trainer(args, best_model, i+1, model_name=args['model_name'])
...
def Main(args, model_name):
    ...
    sys.stdout = DualLogger(f"{path}/{exp_type}.txt")  # 표준 출력 리디렉션
