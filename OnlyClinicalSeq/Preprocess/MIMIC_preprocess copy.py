...
def main(args):
    ...
    for d_name in sorted(os.listdir(f"{args['save_data']}/sequence")):
        ...
        for idx, domain_name in enumerate(args['domain']):
            ...
            a = domain.drop_duplicates(subset=['subject_id', 'hadm_id'])
    ...
    # if args['over_age']: dataset=over_age(dataset) # admit 나이 기준
    # if args['over_height_over_weight']: dataset=over_height_over_weight(dataset)
    # if args['over_24_icustay']: dataset=over_24_icustay(dataset, args["icustay_day"]) 
